...
trait RendersLanguageSwitcher
{
    ...
    private static function dropdown(array $classes, array $languages, string $current): string
    {
        ...
        $out .= '<button id="' . esc_attr($buttonId) . '" type="button" class="wat-switcher-toggle" aria-label="' . esc_attr(sprintf(
            /* translators: %s: active language name. */
            __('Huidige taal: %s. Taalmenu openen', 'webactueel-translate-language-dropdowns'),
            $activeNative
        )) . '" aria-haspopup="menu" aria-controls="' . esc_attr($menuId) . '" aria-describedby="' . esc_attr($helpId) . '" aria-expanded="false">' . self::label('flags_name', $activeCode, $activeNative, $activeFlag) . '<span class="wat-switcher-chevron" aria-hidden="true">▾</span></button>';
        $out .= '<ul id="' . esc_attr($menuId) . '" class="wat-switcher-menu" role="menu" aria-orientation="vertical" aria-labelledby="' . esc_attr($buttonId) . '" hidden>';
    ...
    {
        $out = '<nav class="' . esc_attr(implode(' ', $classes)) . '" aria-label="' . esc_attr__('Taal kiezen', 'webactueel-translate-language-dropdowns') . '"><ul role="list">';
        foreach ($languages as $language) {
