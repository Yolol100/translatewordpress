...
trait CsvEndpoints
{
    ...
    public function xliff_export(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        $rawLanguages = $request->get_param('languages');
        if (is_string($rawLanguages)) {
            $rawLanguages = explode(',', $rawLanguages);
        ...
        if ($xliff === '') {
            return new WP_Error('wat_xliff_export_unavailable', __('XLIFF export vereist de PHP DOM/ext-xml extensie.', 'webactueel-translate-language-dropdowns'), ['status' => 500]);
        }

        $response = $this->raw_export_response($xliff);
        $response->set_headers([
    ...
    /**
     * Build a REST response that can be served as a raw file download by
     * `serve_raw_export()` before WordPress JSON-encodes the response body.
     */
    private function raw_export_response(string $content): WP_REST_Response
    {
    ...
    /**
     * Serve CSV/XLIFF REST exports as raw downloads instead of JSON strings.
     *
     ...
     * @param mixed $server  REST server object.
     */
    public function serve_raw_export($served, $result, $request, $server): bool
    {
        ...
        if (! is_array($data) || empty($data['wat_raw_export']) || ! is_string($data['content'])) {
            return false;
        }
        ...
        foreach ($result->get_headers() as $name => $value) {
            ...
            if (! isset($allowedHeaders[$headerName]) || ! preg_match('/^[a-z0-9-]+$/', $headerName)) {
                continue;
            }

            $safeName = $allowedHeaders[$headerName];
            $headerValues = is_array($value) ? $value : [$value];
            foreach ($headerValues as $index => $singleValue) {
                if (! is_scalar($singleValue)) {
                    continue;
                }

                $safeValue = str_replace(["\r", "\n"], '', (string) $singleValue);
                header($safeName . ': ' . $safeValue, $index === 0);
            }
        }

        echo $data['content']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- File export content is created by the plugin exporter and must remain raw.
        return true;
    }
