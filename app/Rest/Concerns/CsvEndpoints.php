...
trait CsvEndpoints
{
    ...
    public function serve_raw_export($served, $result, $request, $server): bool
    {
            ...
            if (is_array($value)) {
                foreach ($value as $singleValue) {
                    if (is_scalar($singleValue)) {
                        header($name . ': ' . (string) $singleValue, false);
                    }
                }
                continue;
            }
        ...
        echo $data['content']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- File export content is generated by the plugin exporter and must remain raw.
        return true;
    }
