    ...
    public function import_file(string $path, string $delimiter = ',', array $languages = []): array
    {
        ...
        while (($row = fgetcsv($handle, 0, $delimiter, '"', '')) !== false) {
            ...
            if (isset($seen[$rowKey])) {
                $skipped++;
                $errors[] = "Regel {$line}: dubbele hash/language combinatie in import overgeslagen.";
                continue;
            }
            $seen[$rowKey] = true;
            ...
            $stringId = (int) $wpdb->get_var($wpdb->prepare("SELECT id FROM `" . esc_sql(Tables::strings()) . "` WHERE hash = %s", $hash));
            if (! $stringId) {
